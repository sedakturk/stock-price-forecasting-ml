...
def prepare_econometric_data(ticker_symbol: str, start_date: str = '2022-01-01') -> pd.DataFrame:
    """Download data, compute stationary log-returns and lagged features."""
    ...
    # 1. Stationary Target: Daily Log Return
    df['Log_Return'] = np.log(df['Close'] / df['Close'].shift(1))

    # 2. Econometric Lagged Features (AR components)
    for lag in [1, 2, 3, 5, 10]:
        df[f'Return_Lag_{lag}'] = df['Log_Return'].shift(lag)

    # 3. Rolling Technical Features on Stationary Data
    df['Rolling_Vol_10'] = df['Log_Return'].rolling(window=10).std()
    df['Rolling_Mean_10'] = df['Log_Return'].rolling(window=10).mean()
...
def main():
    ...
    # Test Set Return Predictions
    pred_returns_test = model.predict(X_test_scaled)

    # Reconstruct Predicted Prices on Test Set
    ...
    # Model Evaluation Metrics on Price Level
    ...
    # Out-of-Sample 45-Day Iterative Forecast
